    ...
    handleKeyDownEvent = (event) => {
        ...
        if (this.state.problem.content[this.state.selectedGrid.row_index][this.state.selectedGrid.col_index] === "0") {
            if (this.state.gridValues !== null && this.state.selectedGrid.row_index !== -1 && this.state.selectedGrid.col_index !== -1 && (event.keyCode >= 48 && event.keyCode <= 57) || (event.keyCode >= 96 && event.keyCode <= 105)) {
                let vaildNum = true;
                let startI =this.state.selectedGrid.row_index%3;
                if(startI === 0){
                    startI = this.state.selectedGrid.row_index;
                } 
                if(startI === 1){
                    startI = this.state.selectedGrid.row_index - 1;
                } 
                if(startI === 2){
                    startI = this.state.selectedGrid.row_index -2 ;
                } 


                let startJ =this.state.selectedGrid.col_index%3;
                if(startJ === 0){
                    startJ = this.state.selectedGrid.col_index;
                } 
                if(startJ === 1){
                    startJ = this.state.selectedGrid.col_index - 1;
                } 
                if(startJ === 2){
                    startJ = this.state.selectedGrid.col_index - 2 ;
                } 
                for (let i = 0; i < 3; i++){
                    for (let j = 0; j < 3; j++){
                        console.log(startI + i, startJ + j);
                        if(this.state.gridValues[startI + i][startJ + j] === event.key ){
                            vaildNum = false;
                            console.log("FailedNum");
                        

                            // this.setState({
                            //     conflicts: [...this.state.conflicts , { row_index: startI + i, col_index: startJ + j}]
                            // });
                            
                        }
                    }

                    // console.log(this.state.gridValues[this.state.selectedGrid.row_index][i] );  
                }
                for (let k = 0; k < 9; k++){
                    if(this.state.gridValues[this.state.selectedGrid.row_index][k] === event.key ){
                        vaildNum = false;
                        console.log("FailedNum");
                        // this.setState({
                        //     conflicts: [...this.state.conflicts , { row_index: this.state.selectedGrid.row_index, col_index:k}]
                        // });
                        
                       
                        
                    }
                    if(this.state.gridValues[k][this.state.selectedGrid.col_index] === event.key ){
                        vaildNum = false;
                        console.log("FailedNum");
                        // this.setState({
                        //     conflicts: [...this.state.conflicts , { row_index: k, col_index: this.state.selectedGrid.col_index}]
                        // }); 
                    }
                }
                if(vaildNum){
                    let after =  this.state.gridValues;
                    after[this.state.selectedGrid.row_index][this.state.selectedGrid.col_index] = event.key;
                    // this.setState({
                    //     gridValues: after,
                    // });
            ...
            if (this.state.gridValues !== null && this.state.selectedGrid.row_index !== -1 && this.state.selectedGrid.col_index !== -1 ) {
                if(num >= 0 || num <= 9){
                    ...
                    for (let i = 0; i < 3; i++){
                        for (let j = 0; j < 3; j++){
                            console.log(startI + i, startJ + j);
                            if(this.state.gridValues[startI + i][startJ + j] === num.toString()){
                                vaildNum = false;
                                console.log("FailedNum");
                                // this.setState({
                                //     conflicts: [...this.state.conflicts , { row_index: startI + i, col_index: startJ + j}]
                    ...
                    // for (let k = 0; k < 9; k++){
                    //     if(this.state.gridValues[this.state.selectedGrid.row_index][k] ===  num.toString()  || this.state.gridValues[k][this.state.selectedGrid.col_index] === num.toString()  ){
                    //         vaildNum = false;
                    //         console.log("FailedNum");
                    //         break;
                    //     }
                    // }
                    for (let k = 0; k < 9; k++){
                        if(this.state.gridValues[this.state.selectedGrid.row_index][k] ===  num.toString()  ){
                            vaildNum = false;
                            console.log("FailedNum");
                            // this.setState({
                            //     conflicts: [...this.state.conflicts ,{ row_index: this.state.selectedGrid.row_index, col_index:k}]
                        ...
                        if(this.state.gridValues[k][this.state.selectedGrid.col_index] ===  num.toString()  ){
                            vaildNum = false;
                            console.log("FailedNum");
                            // this.setState({
                            //     conflicts: [...this.state.conflicts , { row_index: k, col_index: this.state.selectedGrid.col_index}]
                    ...
                    // for (let i = 0; i < 9; i++){
                    //     console.log(this.state.gridValues[this.state.selectedGrid.row_index][i] );
                    //     if(this.state.gridValues[this.state.selectedGrid.row_index][i] === num.toString() || this.state.gridValues[i][this.state.selectedGrid.col_index] === num.toString() ){
                    //         vaildNum = false;
                    //         console.log("FailedNum");
                    //         break;
                    //     }
                    ...
                    if(vaildNum){
                        let after =  this.state.gridValues;
                        after[this.state.selectedGrid.row_index][this.state.selectedGrid.col_index] = num;
                        this.setState({
                            gridValues: after,
                        });
